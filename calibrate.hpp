...
void calibrate(const cv::Point2f &zvp, const std::vector<cv::Point2f> &hvp, const float &width, const float &height,
               float &focal, std::vector<cv::Point2f> &manh_vps, float &confident)
{
    int infty = 4;
    float accuracy = 2;
    //nt = 2^(5+min(accuracy, log2(width)-5));
    auto nt = std::pow(2., 5 + std::min(accuracy, std::log2(width) - 5));
    auto step_h = width / nt;
        ...
        for (int v2 = v + 1; v2 < hvp.size(); ++v2)
        {
            ...
            if (ff > 0)
            {
                ...
                if (ff > 0.28 * width && ff < 3.8 * width)
                {
                    ...
                    //vv1 = [x1/ff;y1/ff;1];
                    cv::Point3f vv1{x1 / ff, y1 / ff, 1.};
                    //vv2 = [x2/ff;y2/ff;1];
                    cv::Point3f vv2{x2 / ff, y2 / ff, 1.};
                    //vv3 = cross(vv1,vv2);
                    ...
                    auto at_vz2 = std::atan(zvp.y / height) / Dt_h;
    ...
    if (mht.size())
    {
        ...
        if (vvpc_min < 2 or std::abs(zvp.y - v0) >= infty * height)
        {
            if (vvpc_min < 2)
            {
                confident = 3;
    ...
    if (confident == -1 and hvp.size() and (std::abs(zvp.y - v0) < infty * height))
    {
        //id_finite = find(abs(hvp(:,1)-u0) < infty*width);
        auto id_finite = std::find(hvp.begin(), hvp.end(), [](const auto &a) { return (a.y - u0) < infty * width; });
        //if length(id_finite) > 0
        if (id_finite != hvp.end())
        {
            //y1 = hvp(id_finite(1),2) - v0;
            auto y1 = id_finite->y - v0;
            //y2 = zvp(2) - v0;
            auto y2 = zvp.y - v0;
            //ff = sqrt(-y1*y2);
            auto ff = -y1 * y2;
            ...
            if (ff > 0)
            {
                ...
                if (ff > 0.28 * width and ff < 3.8 * width)
                {
                    ...
                    cv::Point3f r2{id_finite->x, id_finite->y, focal};
                    //r2 = r2/norm(r2);
                    r2 /= cv::norm(r2);
                    //r3 = [zvp(1)-u0; zvp(2)-v0; ff];
                    cv::Point3f r3{zvp.x - u0, zvp.y - u0, ff};
                    //r3 = r3/norm(r3);
                    r3 /= cv::norm(r3);
                    //vpx = K*cross(r2,r3);
                    cv::Vec3f cross(r2.cross(r3));
                    cv::Mat vpx = K * cross;
                    //vpx = vpx/vpx(3);
                    auto vp_x = vpx.at<float>(0, 0) / vpx.at<float>(0, 2);
                    auto vp_y = vpx.at<float>(0, 1) / vpx.at<float>(0, 2);
                    //manh_vps = [vpx(1)+u0 vpx(2)+v0; hvp(id_finite(1),1) hvp(id_finite(1),2); zvp(1) zvp(2)];
                    manh_vps.emplace_back(cv::Point2f{vp_x + u0, vp_y + v0});
